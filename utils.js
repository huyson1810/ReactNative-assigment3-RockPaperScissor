export const CHOICES = [
  ...
  export const getStatusString = ({ userChoice, computerChoice, userStatus }) => {
    if (userStatus === "init") {
      return "Rock Paper Scissors by HuySon"
    }
    if (userStatus === "win") {
      return `${capitalize(userChoice)} covers ${capitalize(computerChoice)}. You Win :D`
    }
    else if (userStatus === "lose") {
      return `${capitalize(computerChoice)} covers ${capitalize(userChoice)}. Computer Win :(`
    }
    else {
      return `${capitalize(computerChoice)} with ${capitalize(userChoice)}. Tie :)`
    }
  }